...
## Импортировать наборы данных, необходимых для главы 5
...
png(filename=file.path(PSDS_PATH, 'figures', 'psds_05_naive_bayes.png'),  width = 4, height=3, units='in', res=300)
...
ggplot(data.frame(borrower_score=c(0,1)), aes(borrower_score)) +
  ...
  labs(y='Вероятность', x='Балльная оценка заемщика')
dev.off()


## Фрагмент кода для LDA
...
## Фрагмент кода для рисунка 5-1: LDA
...
ggplot(data=df, aes(x=logit, y=odds)) +
  geom_line() +
  labs(x = 'log(отношение шансов)', y='отношение шансов') +
  ylim(1, 100) +
  xlim(0, 5) +
  theme_bw()
dev.off()
...
ggplot(df, aes(x=payment_inc_ratio, y=partial_resid, solid = FALSE)) +
  ...
  labs(y='частный остаток', x='платеж к доходу') +
  xlim(0, 25) +
  theme_bw()
dev.off()
...
ggplot(roc_df, aes(x=specificity, y=recall)) +
  ...
  theme_bw() +
  labs(y='полнота', x='специфичность') 
dev.off()


## Фрагмент кода для рисунка 5-7
...
## bof???
## Фрагмент кода для понижающего отбора
full_model <- glm(outcome ~ payment_inc_ratio + purpose_ + 
                    home_ + emp_len_+ dti + revol_bal + revol_util,
                  data=train_set, family='binomial')     # train_set????
pred <- predict(full_model)
mean(pred > 0)

## Фрагмент кода для повышающего отбора/повышающей перевесовки
...
## Пример SMOTE (генерация данных); пример не из книги
library(unbalanced)
loan_data_samp <- sample_frac(loan_data, .05)   # UPD loan_all_data -> loan_data
smote_data <- ubSMOTE(loan_data_samp, loan_data_samp$outcome, 
                      perc.over = 2000, k = 5, perc.under = 100)

# Фрагмент кода для рисунка 5-8: сравнение методов
...
labs <- data.frame(x=c(.375/2, .45, 1.525/2, 1.625/2, .575, .525/2),
                   ...
                   decision = factor(c('невозврат', 'погашено', 'погашено', 'погашено', 'невозврат', 'невозврат')))
...
impure <- data.frame(p = rep(x, 3),
                     impurity = c(2*x,
                                  ...
                                  info(x)),
                     type = rep(c('Точность', 'Джини', 'Энтропия'), rep(51,3)))
...
# Хороший график, демонстрирующий градиент предсказаний, но не показателен как предыдущий график
...
imp <- data.frame(Predictor = rep(nms, 2),
                  ...
                  Type = rep( c('Уменьшение точности', 'Уменьшение Джини'), rep(nrow(imp1), 2)))
